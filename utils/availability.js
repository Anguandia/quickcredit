...
export const loanNot = function (req, res, next) {
  const key = req.params.loanId ? req.params.loanId : req.body.client;
  const field = req.params.loanId ? 'id' : 'client';
  pool.connect((error, client) => {
    client.query(`SELECT * FROM loans WHERE ${field}='${key}'`, (err, result) => {
      if (err) {
        res.status(500).json({ status: 500, error: 'internal error' });
      } else if (result.rows.length > 0 && field == 'client') {
        if (result.rows.some(v => v.repaid == false)) {
