...
// these tests will only cover authentication failures, successful authentication tests are incoporated in each rotes tests in the testtest_users and testLoans modules
describe('test route authentication', () => {
    before((done) => {
        // clear the test_users' array
        users.splice(0);
        loans.splice(0);
        // signup 3 test test_test_users to get a token
        chai.request(app)
